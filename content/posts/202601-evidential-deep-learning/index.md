...
In standard classification tasks, we are all accustomed to treating the network's normalized probability vector as a proxy for the network's confidence. When the network is confident that an instance belongs to a particular class, the network assigns a high output to that class, like so:

$$
{\mathbf{p}} = [0.9, 0.06, 0.04]
$$

The normal softmax activation and cross-entropy loss distribute “confidence” across the classes and force the network to assign at least some confidence to each class. We have all treated this as the expected behavior of a deep neural network.

However, consider this case: what happens if we feed an image of a cat into a network trained on MNIST? There’s a high chance it would say this is an image of the number “7”, even though it is completely ignorant of what is actually in the image. Or, consider the following handcrafted out-of-distribution image from the MNIST dataset:

{{< figure src="ood-images1.png" caption="Out-of-distribution MNIST sample (from [here](https://muratsensoy.github.io/uncertainty.html))" >}}


Which class would the third image belong to? The network could produce a distribution indicating equal confidence that the picture belongs to both classes “0” and “6”. However, in a typical classification task, we only expect the neural network to be confident in one class per image instance. Both examples show that we need some form of quantification for *uncertainty*.

This is where **Evidential Deep Learning** [1] comes into play. Instead of modeling **probability**, the NN now acts as a sensor gathering **evidence**. In the above cases, instead of spitting out a uniform categorical distribution (blind guess), we make the network output “I’m *not sure*, I see *no* *evidence*” (remember the italic parts for later).

Under this paradigm, the network is incentivized to assign zero evidence value to non-label classes, which, assuming clean training data, lack sufficient supporting features in the input image data. As a result, it avoids blind guesses when actual out-of-distribution samples appear during inference, which do not have sufficient supporting features for all learned classes.

The original paper is quite insightful, but I found the bar too high for readers unfamiliar with all the mathematical concepts. While we still aim to maintain a balance with the mathematical details, this blog post is intended to provide a higher-level view of the method and to begin from the starting point we are all familiar with: building an image classifier.

## Beyond softmax

Let’s formulate our problem again. Suppose that we are building a classifier to assign a class $k=1..K$ to an image $\mathbf{x}_i$. After going through a backbone network (could be anything — FNN, CNN, or ViT), the network outputs the logit vector, for the instance:
...
But this puts us in the situation described earlier: the network is forced to pick a "winner" class, even if it has no idea what is happening.
...
The mathematical details are indeed scary and can be spared for later. But roughly speaking, we modify the network to output an **evidence** vector $\mathbf{e}_i \in [0, \infty)$ akin to a rectified logit activation. From this **evidence**, we derive not only the class probabilities but also a distinct scalar quantifier of uncertainty $0 < u < 1$. 

The expectation for this new architecture/paradigm is intuitive:

1. **Knowledgeable behavior:** it should behave similarly to softmax when it’s confident. When the network “successfully” predicts a correct example, the evidence $e_{ik}$ for that class is strong at $k=\mathrm{argmax}(\mathbf y_i)$, and low everywhere else. At the same time, the uncertainty term $u_i$ should also be low.
2. **Ignorant behavior**: this is where EDL diverges from softmax. Consider the situation where the network is clueless. A softmax network might produce a uniform output $\mathbf p = [0.33, 0.33, 0.33]$. In contrast, a network trained with evidential objectives expresses ignorance by outputting zero evidence $\mathbf{e} = [0, 0, 0]$ (which corresponds to *“I see no evidence”*). This state mathematically forces the uncertainty term to be $u=1.0$ (corresponding to *“I’m not sure”*).


In light of this, EDL treats the neural network as a sensor that collects **evidence** to form a "subjective opinion". Modeling evidence instead of probability directly allows us to separate **aleatoric uncertainty** (confusion between known classes, e.g., a handwritten ‘1’ that looks like a ‘7’) from **epistemic uncertainty** (total lack of knowledge, e.g., the cat image). 
...
Since the essence of every learning task is determined by its loss function, it is natural to discuss this early on. The objective for evidential learning is a compound loss of a *risk term* and a *regularization term*:
...
In the theory of Subjective Logic, evidence serves as the **parameters** for a Dirichlet distribution, which is simply:
...
The uncertainty $u_i = K/S_i$ comes as a side effect and does not contribute to the learning process.

Note that at the moment, it appears we are adding extra steps just to obtain a normalized score $\hat p_{ik}$ from an activation function. The real guts, however, lie in the loss functions. But now there’s too much new math! To understand what the hell is going on, we should take a step back for a moment. Spend some time internalizing how the loss function could be computed from the logits $\mathbf a_i$ before continuing.
...
1. **The “I don’t know” case**: if the network finds zero evidence for any class ($e_{ik} \approx 0$ for all $k$), then every parameter $\alpha_{ik} \approx 1$, which leads to $S_i \approx \sum_{k=1}^K1=K$. Hence:
...
1. **The “confident” case:** Recall that $S_i=\sum_k (e_{ik}+1)$. As the network learns to gather evidence for a class (e.g., learning the features to classify a ‘7’ in MNIST), the corresponding $e_{ik}$ grows. As $S_i \to \infty$, $u_i \to 0$.

So, in a well-trained evidential network, $S_i$ acts as a proxy for **total confidence**. If it is large, the network is knowledgeable; otherwise, it is ignorant.

### How is no-evidence modeled?

The first point in the previous section relies on a property: if the network finds zero evidence for any class, then $e_{ik} \approx 0$ for all $k$. How do we force the network to output zero evidence when it is confused? 

Standard neural networks have no incentive to do this. Remember that the standard cross-entropy loss is:
...
which restricts the update of the neural network’s weights to depend solely on the confidence of the correct class. This objective allows the model to output arbitrary probabilities for other classes without penalty. During inference, a high predicted probability assigned to non-label classes also signals uncertainty regarding the input data.

In EDL, we effectively force the **evidence** of non-label classes to zero during training. We do this by pulling the predictive distribution towards the **Uniform Dirichlet distribution $D(\cdot | \mathbf 1)$**, the state of *complete ignorance*, where the parameters are $\alpha_{ik}=1 \Leftrightarrow e_{ik} = 0$. 

This “pull” is implemented through the **regularization component** of the loss function:
...
The network is now incentivized to assign zero evidence value to non-label classes, which, assuming clean training data, lack sufficient supporting features in the input image data. As a result, it avoids blind guesses when actual out-of-distribution samples appear during inference, which do not have sufficient supporting features for all learned classes.


{{< admonition type=info title="Reminder" open=true >}}

**Up to now, we have completely answered the following questions:**
